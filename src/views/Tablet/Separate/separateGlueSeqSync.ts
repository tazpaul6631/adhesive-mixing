...
/** API nguồn map seq cho bảng keo không trộn. */
export const resolveNoMixApiItemsForSeqMap = (
  ...
  if (isNoMixGlue) {
    return Array.isArray(respData?.noSeparateGlues) ? respData.noSeparateGlues : [];
  }

  if (Array.isArray(respData?.noSeparateGlues) && respData.noSeparateGlues.length > 0) {
    return respData.noSeparateGlues;
  }

  return (respData?.separateGlues || []).filter(
    (item: any) => String(item?.glueId ?? item?.materialCode ?? '') === String(noMixGlueId)
  );
};
